...
pub fn transcribe(
    ...
    if let Some(_diarize_options) = diarize_options {
        // TEMPORARY: Diarization disabled for initial build due to pyannote-rs compilation issues
        tracing::warn!("Diarization temporarily disabled - falling back to simple transcription");
    }
    
    // Use simple transcription (fallback when diarization disabled or not requested)
    {
